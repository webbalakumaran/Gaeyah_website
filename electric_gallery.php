...
<html lang=en>
   <head>
      ...
      <style>.thumb{margin-bottom:15px}.thumb:last-child{margin-bottom:0}.thumb figure img{-webkit-filter:grayscale(100%);filter:unset;-webkit-transition:.3s ease-in-out;transition:.3s ease-in-out}</style>
   </head>
   <body style=padding-top:2%;padding-bottom:7%>
      ...
      <script>$(document).ready(function(){$(".gallery").magnificPopup({delegate:"a",type:"image",tLoading:"Loading image #%curr%...",mainClass:"mfp-img-mobile",gallery:{enabled:true,navigateByImgClick:true,preload:[0,1]},image:{tError:'<a href="%url%">The image #%curr%</a> could not be loaded.'}})});</script>
   </body>
</html>